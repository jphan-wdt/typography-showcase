...
export default function StickyPanel({
  ...
  const textLayout = (side) => {
    ...
    return (
      <motion.div
        className={`top-0 w-full flex flex-col gap- justify-between ${alignLeft} text-white font-custom font-extrabold tracking-normal`}
        style={left}
      >
        <div
  ...
  const imageLayout = () => (
    <motion.div
      className={`top-0 h-full  w-full flex flex-col gap-5`}
      style={right}
    >
  ...
  return (
    <div
      ...
      ref={scrollRef}
    >
      {leftLayout === "text" ? textLayout("left") : imageLayout()}
      {rightLayout === "text" ? textLayout("right") : imageLayout()}
    </div>
  );
}
